...
/* in rem */
const radiusValue = z.string().regex(
    /^\d*\.?\d+rem$/,
    'Invalid radius format (expected rem)',
);
...
export const shadcnStylesSchema = z.object({
    ...
    fontSans: z.string().describe('A valid google font id, e.g. "fira-code" best suitable for given theme, font ID, not name'),
