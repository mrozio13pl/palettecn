...
export const generateThemeTool = tool({
    ...
    execute: async (_, { messages, experimental_context, toolCallId, abortSignal }) => {
        try {
            const id = generateId();
            const writer = (experimental_context as any).writer as UIMessageStreamWriter<ChatMessage>;
            ...
            const { partialOutputStream, output: finalObject } = streamText({
                ...
                output: Output.object({
                    ...
                    description: 'The theme must include both and only \'light\' and \'dark\' modes at top level, like { name: "short name", dark: {...}, light: {...} } with all required CSS variables written in oklch.',
                }),
                abortSignal,
                system: `
