...
function FontPicker({ font, onFontChange }: { font?: string; onFontChange: (font: string) => void }) {
    ...
    const filteredFonts = useMemo(() => {
        ...
        if (query.trim().length) searchedFonts = fuzzysort.go(query, fonts, { keys: ['id', 'family'] }).map(({ obj }) => obj);
        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button className="w-full justify-between" variant="outline">
                    {font ? fonts.find(f => font === f.id)?.family : <p className="text-muted-foreground">Select font</p>}
            ...
            <PopoverContent className="w-96 space-y-2">
                ...
                <ScrollArea className="h-80 space-y-1">
                    {filteredFonts?.map(fontItem => (
                        <PopoverClose asChild key={fontItem.id}>
                            <Button
                            ...
                            >
                                <h3
                                    className="text-lg"
                                    style={{
                                        fontFamily: loadedFonts.has(fontItem.id) ? fontItem.family : undefined,
                                    }}
                                >
                                    {fontItem.family}
                                </h3>
                                <p className="text-muted-foreground text-xs font-normal">{fontItem.category}</p>
                            </Button>
                        </PopoverClose>
                    ))}
                </ScrollArea>
